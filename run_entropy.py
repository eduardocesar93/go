...
def main():
    ...
    for entropy_index in range(len(ENTROPY_DIST)):
        plt.xlabel('Entropy')
        plt.ylabel('Density Funtion')
        plt.title('Entropy Distribuition in position {0}'.
                  format(entropy_index + 1))
        plt.hist(ENTROPY_DIST[entropy_index], density=True, bins='auto')
        plt.savefig('{0}/{1}/entropy/{2}{3}.png'.format(DIRECTORY, directory,
                    'sit_', entropy_index + 1))
        plt.clf()
