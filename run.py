...
def run(input_dir, output):
    ...
    limit_rank = 1000
    matrix_values = [[i, stats.matrix_values[i]] for i in range(len(stats.matrix_values))]
    matrix_values.sort(reverse=True, key=lambda x: x[1])
    matrix_values = matrix_values[0:limit_rank]
    plot.save_scatter_csv([[i[0] for i in matrix_values], [i[1] for i in matrix_values]], results_dir,
                          'Positions', labels=['Value', 'Frequency'])
    valid_positions = [i[0] for i in matrix_values]
    plot.scatter_plot('Positions', [[i + 1 for i in range(len(matrix_values))], [i[1] for i in matrix_values]],
                      results_dir, 'Positions', 'Position', 'Frequency', 30, 20, linear_regression=False)
    plot.scatter_plot('Positions', [[i + 1 for i in range(len(matrix_values))], [i[1] for i in matrix_values]],
                      results_dir, 'Positions', 'Position', 'Frequency', 30, 20, linear_regression=False, log=True)
    stats.transition_matrix = utils.clean_matrix(stats.transition_matrix, valid_positions)
    plot.save_matrix_csv(stats.transition_matrix, results_dir, "Transition Matrix", labels=False)
    plot.colormap(stats.transition_matrix, results_dir, "Transition Matrix", "Second Position",
    ...
    plot.save_matrix_csv(stats.transition_matrix, results_dir, "Transition Matrix Ordered", labels=False)
    plot.colormap(stats.transition_matrix, results_dir, "Transition Matrix", "Second Position",
                  "First Position", width=300, height=300, percentile=98)
